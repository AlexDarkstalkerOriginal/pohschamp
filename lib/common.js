  ...
  data: {      
   ...
   methods: {
    monsterPage: function(id, owner) {
      ...
      // var args2 = [];
      // args2.push(owner);
      // var callArgs2 = JSON.stringify(args2);    
      // var callFunction2 = 'getUserHistory';                  
...
// страница боя
  Vue.component('monster-fight', {
    props: ['name', 'src', 'id', 'win', 'exp', 'drop', 'owner', 'price', 'result', 'boolen', 'draw'],
    template: `<div class="monster">\
                <h3>{{name}}</h3>\
                <img v-bind:src="src" alt="">\                
                <a href="#" class="info" v-on:click="vm.monsterPage(id,owner)"><img src="img/info.png" alt=""></a>\
...
// мои монстры лаб  
  Vue.component('my-monster-lab', {
    props: ['name', 'src', 'id', 'win', 'exp', 'drop', 'sale', 'owner'],           
    template: `<div class="monster">\                
...
// страница монстра гет
  ...
  function cbMonsterPage(resp) {
     var page_monster = JSON.parse(resp.result);    
      ...
      page_monster.name = fix_name(page_monster.name);
...
// страница боя
  function cbFightRender(resp) {    
    ...
    var last_index = fight_arr.length - 1;
    var fight_res = fight_arr[last_index].RESULT;
    var attacker = fight_arr[last_index].atacker;
    var defender = fight_arr[last_index].defender;
...
// история боев кэпса
  ...
  //    console.log('history page ' + JSON.stringify(resp));
